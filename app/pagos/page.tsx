...
export default function PagosPage() {
  ...
  const [valorUdiActual, setValorUdiActual] = useState<number | null>(null)
  ...
  const cargarValorUdi = async () => {
    ...
    try {
      ...
      setValorUdiActual(udi.valor)
      setValorUdi(udi.valor.toString())
    } catch (error: any) {
      setValorUdi("")
    ...
    <div className="min-h-screen bg-background">
                      ...
                      {pagosDia.slice(0, 2).map((pago) => {
                        ...
                        return (
                          <button key={pago.id} onClick={() => pago.estatusCobranza !== "pagado" && abrirRegistro(pago)} className="block w-full text-left text-xs p-1 rounded bg-muted/50 truncate" style={{ borderLeft: `3px solid ${pago.companiaColor}` }}>
                            ...
                            {pago.moneda === "UDIS" && pago.estatusCobranza !== "pagado" && valorUdiActual && ` · $${(pago.monto * valorUdiActual).toLocaleString("es-MX", { maximumFractionDigits: 2 })}`}
                ...
                <tbody>
                  {pagosView.map((pago, index) => (
                    <motion.tr key={pago.id} className="border-b border-border/30 hover:bg-muted/50 transition-colors" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.02 }}>
                      ...
                      <td className="p-4"><p className="font-semibold">{pago.monto.toLocaleString()} {pago.moneda}</p>{pago.moneda === "UDIS" && pago.estatusCobranza !== "pagado" && <p className="text-xs text-muted-foreground">{cargandoUdi ? "Consultando UDI actual..." : valorUdiActual ? `$${(pago.monto * valorUdiActual).toLocaleString("es-MX", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} MXN · UDI actual ${valorUdiActual.toFixed(6)}` : "UDI actual no disponible"}</p>}</td>
      ...
      <Dialog open={!!pagoSeleccionado} onOpenChange={open => { if (!open) setPagoSeleccionado(null) }}>
        <DialogContent className="sm:max-w-[440px]">
          ...
          {pagoSeleccionado?.moneda === "UDIS" && <div className="space-y-2 rounded-lg bg-muted/40 p-3"><Label>Valor UDI actual (Banco de México)</Label>{cargandoUdi ? <p className="text-sm text-muted-foreground">Consultando valor actual...</p> : valorUdi && Number(valorUdi) > 0 ? <p className="text-sm">UDI {Number(valorUdi).toFixed(6)} · Equivalente: ${(pagoSeleccionado.monto * Number(valorUdi)).toLocaleString("es-MX", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} MXN</p> : <div className="flex items-center gap-2"><p className="text-sm text-destructive">{errorUdi || "Valor no disponible"}</p><Button size="sm" variant="outline" onClick={() => void cargarValorUdi()}>Reintentar</Button></div>}</div>}
